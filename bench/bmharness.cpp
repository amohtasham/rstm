...
//#include "../stamp-0.9.10/lib/controller.h"
...
volatile bool_t   global_timedExecution = TRUE;
volatile bool_t   global_startExecution = TRUE;
...
extern "C" void catch_SIGALRM(int) {
#ifdef DYNAMIC
    if (++lookupSequenceIndex == SEQUENCE_LENGTH)
    {
        CFG.running = false;
    ...
    {
        CFG.lookpct = lookupPercents[lookupSequence[lookupSequenceIndex]];
        CFG.inspct = (100 - CFG.lookpct)/2 + CFG.lookpct;
        alarm(CFG.duration);
    }
#else
    CFG.running = false;
...
int main(int argc, char** argv) {
    ...
    global_isTerminated = FALSE;
    controller_alloc(CFG.threads);
